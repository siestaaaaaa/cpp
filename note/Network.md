...
1. 短连接：每个请求都会创建一个新的 TCP 连接，并且在请求完成后关闭此连接
2. 请求的队头阻塞：由于短连接的存在，客户端一次只能发送一个请求

## HTTP1.1

1. 默认使用长连接，减少了频繁创建和关闭连接的开销
2. 解决了请求的队头阻塞，但没有解决响应的队头阻塞

## HTTP2.0

1. 基于 HTTPS
2. 头部压缩：对一些重复的头部字段进行压缩，减少传输的流量（双方会协商一个编码规则，然后对头部进行编码）
3. 二进制格式：采用二进制格式传输数据，提高数据传输的效率和报文解析的速度
4. 并发传输：由于二进制分帧的特性，多个流可以在同一条连接上交错传输，接收方根据帧上的流标识符来组装消息
5. 服务器推送：服务器可以主动向客户端发送消息，不再是被动地响应
6. 解决了 HTTP 层的队头阻塞，但还是存在 TCP 层的队头阻塞（如果丢包，需要等待 TCP 重传）

## HTTP3.0

1. 基于 UDP，解决了 TCP 层的队头阻塞
2. 在应用层实现了 QUIC 协议，具有类似 TCP 的连接管理、拥塞控制、流量控制等特性，实现可靠传输
3. 整合 TLS 握手，提高建立连接的速度
4. 使用连接 ID 将不同网络的请求和响应关联起来，使得网络切换无需重新建立连接
...
第四次挥手为什么要等待 2MSL / 主动关闭方为什么要 TIME_WAIT：
...
网络拥堵时，丢包变多，TCP 会进行超时重传，从而加剧网络拥堵情况，导致更多的丢包，陷入恶性循环

所以 TCP 在发送端维护了一个拥塞窗口，根据网络拥堵情况控制发送速度
...
HTTP 的 Keep-Alive 是长连接，可以复用一条 TCP 连接处理多个 HTTP 请求，减少了频繁创建和关闭连接的开销

TCP 的 Keepalive 是 TCP 保活机制，当连接上长时间没有数据时，内核会发送探测报文来检测对方是否还在线，从而判断是否要关闭连接
