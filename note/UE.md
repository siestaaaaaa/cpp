...
属性同步只用于服务器向客户端同步数据，给需要同步的属性标记 Replicated，并实现 GetLifetimeReplicatedProps 函数，当这些属性在服务器上发生变化，就会自动同步到客户端

-- RPC 一般只用于客户端向服务器发送数据

## 移动同步

客户端每帧上报自己模拟的移动结果和用户的移动输入（RPC）

服务器收到后会去模拟角色的移动，如果和客户端上报的差别不大，则会采纳客户端模拟的结果，否则使用服务器模拟的结果

最后，服务器将这帧所有角色的移动结果，同步到所有客户端（属性同步）
