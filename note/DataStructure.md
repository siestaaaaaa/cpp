...
插入删除：数组只有尾部插入删除是 $O(1)$，其他位置插入删除需要移动元素，时间复杂度为 $O(n)$，链表插入删除只需要修改指针指向，时间复杂度是 $O(1)$ 的
...
通过这些特性，红黑树可以确保最长路径不超过最短路径的两倍（路径是从根节点到空结点），从而保证操作的时间复杂度为 $O(logn)$

由于红黑树是近似平衡，所以查找性能略差于 AVL 树，但插入性能优于 AVL 树

# 哈希表

哈希表就是利用哈希函数把键映射到某个位置，然后将键值对存储在这个位置，时间复杂度为 $O(1)$

哈希冲突就是不同的键通过哈希函数映射到了同一个位置，解决办法：开散列，闭散列（线性探测、二次探测等）

负载因子：插入元素的个数 / 散列表长度。负载因子高，哈希冲突的概率较高，建议扩容
