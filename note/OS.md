...
git log origin xxx # 查看远程xxx分支的历史
git rebase origin xxx # 将当前分支重放到远程xxx分支的顶部
git push origin xxx # 将本地提交同步到远程xxx分支
```
...
操作系统会给每个进程分配一套独立的地址空间（每个地址标识 1 字节），每个进程都认为自己独占整个系统的内存资源

地址空间包括用户空间和内核空间，用户态不能访问内核空间，需要通过系统调用进入内核态
...
- 数据段：存放全局数据和静态数据
  ...
  - .bss 段：无初始值，只记录符号和大小，加载时初始化成 0
- 堆：用于动态内存分配，从低地址向高地址增长，由程序显式分配释放，可能产生内存碎片
- 栈：存储函数参数、局部变量、返回值等，从高地址向低地址增长，系统自动管理
...
mmap 可以将文件映射到进程地址空间，使得文件读写直接操作内存，避免频繁的 read/write 系统调用

## 地址空间布局随机化

每次启动程序，地址的分配是随机的，这样可以抵御缓冲区溢出攻击，提高安全性

GDB 中默认关闭 ASLR，方便用户定位问题

## 堆内存管理

系统调用：

- brk/sbrk：调整堆顶指针，适合小内存分配
- mmap：映射匿名内存页，适合大内存分配

常见的分配器：

- glibc 的 ptmalloc：128 KB 以下使用 brk，128 KB 以上使用 mmap，通过自由链表管理内存块，在每个内存块的头部存储元信息
- jemalloc/tcmalloc：优化多线程环境下的内存分配效率
...
共享内存就是将同一块物理内存映射到多个进程的地址空间，是最快的进程间通信方式，没有用户态和内核态之间的数据拷贝

共享内存不提供同步机制，需要调用方自己处理

## 信号

信号是一种异步事件通知机制，用于通知进程发生了某种事件
...
索引结点：文件的唯一标识，记录文件的元信息（权限、大小、数据块指针等，不包含文件名）
