...
每个类型都有自己的对齐要求，对齐要求是 2 的幂，可以用 alignof 获取（C++11）
...
static 修饰全局变量或函数：内部链接，可以用来防止命名冲突（内部链接的还有匿名命名空间、const 全局变量）

static 修饰类的成员变量：静态成员变量不与对象绑定，需要单独定义，可以直接用类名访问。static 关键字只用于类中声明
...
数据段存放的是全局数据和静态数据，根据是否有初始值分为 .data 段和 .bss 段（程序加载时统一初始化为 0，不占程序文件的空间）
...
虚函数可以被 inline 修饰，虽然 inline 的语义最早是建议编译器内联优化，但在 C++98 已经改为允许一个符号在不同编译单元多次定义、一般用来修饰头文件中定义的全局变量或函数

至于虚函数是否能被内联优化，当虚函数是动态绑定（通过基类指针或引用调用虚函数）的时候不会被内联优化，因为动态绑定在运行时才确定要调用的函数
