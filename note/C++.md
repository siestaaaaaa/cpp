...
new 先调 operator new() 分配内存（分配失败会抛 std::bad_alloc 异常），再调构造函数构造对象（如果构造函数抛异常会自动调 operator delete() 释放内存）

delete 先调析构函数析构对象（析构函数不允许抛异常，就算抛异常也要在析构函数中处理），再调 operator delete() 释放内存
...
RAII 就是把资源和对象的生命周期绑定，在构造函数获取资源、析构函数释放资源
...
派生类成员访问受​继承方式​和​基类成员访问共同影响

- 基类的 private 成员总是不可访问
- 基类的其他成员在派生类的成员访问 = min(继承方式，在基类的成员访问)
...
模板的语法非常复杂，大致可以理解成拼图，这里只讲解一些常用语法
