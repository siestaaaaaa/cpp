...
读取（除了编译时常量）/ 写入 / 取地址 / 绑定到引用，即为 ODR 使用

ODR 使用必须有定义，否则可能链接错误

```cpp
struct X {
    static const int n = 100; //编译时常量 只是类内声明 没有定义
};

std::cout << X::n;  //不是ODR使用 ok
std::cout << &X::n; //是ODR使用 err
```

## inline

inline 的本意是建议编译器优先内联，就是将函数在调用处展开，避免函数调用的开销（栈帧的分配和销毁、跳转对缓存不友好），但可能导致更大的可执行文件

C++98 起，inline 的语义改为允许一个符号在不同编译单元多次定义，链接时只取一份定义，可以用来修饰头文件中定义的全局变量或函数

类中定义的函数是隐式 inline 的

C++17 起，inline 可以修饰类的静态数据成员，使得静态数据成员可以在类中定义（类似的还有 constexpr）

```cpp
struct X {
    static const int i = 100;        //只是类内声明 没有定义
    static const inline int j = 100; //定义
    static constexpr int k = 100;    //定义
};
...
const 局部变量存在栈上；const 全局变量存在只读数据段

static const 变量存在只读数据段，确保不会被修改

## new和delete的原理

new 先调 operator new() 分配内存（失败会抛出 std::bad_alloc），再调构造函数构造对象
...
std::array 是定长数组，是原生数组的封装，没有额外的空间开销

尾部插入删除 $O(1)$，其他位置插入删除 $O(n)$
...
任意位置插入删除 $O(1)$，不需要移动数据

每个结点需要两个指针的额外空间，来维持双向链表结构

#### std::forward_list

std::forward_list 是 std::list 的内存优化版本，单向链表，每个结点需要一个指针的额外空间

#### std::deque

std::deque 是双端队列，内存分段连续

map 数组的每个元素指向一段连续的内存

头尾插入删除 $O(1)$​，其他位置插入删除只需移动较少的一半数据
...
PIMPL（指向实现的指针）可以隐藏类的实现细节，减少编译时依赖，提高编译速度
...
继承是建立类之间关系的一种方式，public 继承中派生类和基类是 is-a 的关系
...
多态就是不同类型的对象调用同一接口，产生不同的行为。多态可以给同一接口提供灵活的实现
...
constexpr 表示常量表达式，允许在编译期求值（consteval 则是必须在编译期求值，否则编译失败）
...
原子操作：不会被打断的操作。std::atomic 的成员函数都是原子的

内存序：保证操作之间的顺序

### 内存序

CPU、缓存和编译器优化都可能导致指令乱序，使用内存序可以保证操作之间的顺序

std::atomic 默认使用最强的 std::memory_order_seq_cst（顺序一致模型），如果没有性能优化的需求，直接使用默认的内存序即可
...
std::string_view 是一个字符串视图，无需拷贝底层数据即可查看字符串
