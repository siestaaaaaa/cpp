...
而在现代 C++ 中，inline 的作用是允许一个符号在不同编译单元多次定义，链接时只取一份定义，一般用来修饰头文件中定义的全局变量或函数。函数模板和类中定义的函数是隐式 inline 的
...
相同作用域中，形参不同的同名函数构成重载，编译器会根据重载决议的规则选择最合适的函数进行调用

不同作用域的同名函数，局部优先，而非重载

运算符重载也是函数重载，只不过函数名是 `operator xxx`
...
进程的用户空间从低到高可分为：代码段、数据段、堆、栈

代码段存放的是机器指令

数据段存放的是全局数据和静态数据，根据是否有初始值分为 .data 段和 .bss 段（不占程序文件的空间，程序加载时统一初始化为 0）

堆和栈的区别：

- 栈从高地址向低地址增长，系统自动分配释放
- 堆从低地址向高地址增长，需要用户自己管理
- 栈空间比较小，分配速度快
- 堆空间比较大，分配速度慢
...
operator new() 和 operator delete() 是可以重载的
...
placement new 就是 operator new() 的一个重载，用于在已分配的内存上构造对象
...
内存泄漏：程序没有释放已经不再使用的内存，导致这部分内存不能再次被使用

内存泄漏的危害：对于长期运行的程序，内存泄漏会导致系统可用的内存越来越少，最后 OOM
...
CRTP 是模板编程的一种惯用手法，就是写一个基类模板，派生类继承时将自身作为基类模板的参数

CRTP 可以实现静态多态：在基类暴露一个方法，在这个方法中调用派生类的实现
...
1. 隐藏类的实现细节
2. 减少编译时依赖，提高编译速度（现在更好的是用预编译头或模块）
...
RAII 是 C++ 资源管理的惯用手法，就是把资源和对象的生命周期绑定，在构造函数获取资源、析构函数释放资源，这样可以保证当对象的生命周期结束时，持有的资源都能正确释放

## 池

池是一种预先创建并复用资源的技术，用户从池中获取资源，用完归还到池中，从而避免频繁创建和销毁资源（空间换时间）
...
- 类作用域外只能访问 public 成员
- 基类的 private 成员对派生类总是不可访问的
- 基类的其他成员在派生类的成员访问 = min(在基类的成员访问，继承方式)
...
虚函数可以在派生类重写（派生类的成员函数，拥有和基类的虚函数完全相同的接口，那么该成员函数也是虚函数并且会覆盖基类的虚函数）
...
- 虚函数是动态绑定的（运行时才确定要调用的函数），编译器不能内联优化
...
基类的析构函数需要是虚函数，否则通过基类指针 delete 派生类对象是未定义行为，可能造成内存泄漏
...
lambda 表达式是一个匿名函数对象，可以捕获**当前作用域**的变量，定义一个 lambda 表达式，同时也隐式定义了一个重载 ```operator()``` 的类，捕获的变量就是这个类的成员变量，函数参数就是 `operator()` 的参数，函数体就是 `operator()` 的函数体
...
在典型实现中，共享指针会保有两个指针：存储的指针和指向控制块的指针。控制块才是对象的管理者，控制对象的生命周期
...
使用构造函数创建共享指针，控制块和被管理对象只能被分别创建（可能抛出异常）

使用 std::make_shared 创建共享指针，会一次性创建控制块和被管理对象（效率更高并且异常安全）

### std::enable_shared_from_this

std::enable_shared_from_this 是一个类模板，允许派生类通过 `shared_from_this()` 生成指向自己的共享指针，生成的共享指针和已有的共享指针共享控制块，这是通过 std::enable_shared_from_this 的一个弱指针成员 weak_this 实现的
...
模板编程的思想在于，把类型当作一种数据，通过模板来传参

模板的语法规则非常复杂，可以大致理解成拼图，这里只讲解一些常用规则
...
模板声明和定义分别写在头文件和源文件，就没有隐式实例化了，链接时会报错说符号未定义

可以在模板定义的地方显式实例化，让编译器生成对应的代码，来解决这个问题（模板全特化有类似的作用）

建议：模板不要分文件，而是直接写在头文件，比如 boost 里的 .hpp
...
概念是 C++20 引入的新特性，可以定义一套约束，用来约束模板接口
...
根据实现的不同，读写锁可以分为读优先、写优先和完全公平，可以用互斥锁实现也可以用自旋锁实现

C++17 引入了 std::shared_mutex，支持完全公平的独占锁定和共享锁定（std::shared_lock），可以用来实现读写锁

pthread 库提供的读写锁是写优先的，要注意使用场景（如果写多于读，读线程可能饿死。读优先同理）

## 乐观锁

前面的互斥锁、自旋锁、读写锁都属于悲观锁，悲观锁认为多线程冲突概率很高，在访问共享资源前都需要先加锁

乐观锁先假设多线程没有冲突，等线程修改完共享资源，再验证这段时间有没有其他线程修改了资源（通过版本号或时间戳），如果没有则成功，否则放弃本次操作、后面再重试
...
只要函数中有 `co_await`、`co_yield`、`co_return` 三个关键字的任意一个，这个函数就是协程
...
工厂模式可以将对象创建和业务逻辑分离，降低耦合度，提高可扩展性
