...
>
> 前言：作为一门全能的编程语言，C++ 非常庞大而复杂，我们只需使用它的一个子集，完成自己的需求就好，而不必过分纠结 C++ 本身的复杂性
...
std::vector 是动态数组，在堆上分配一块足够大的连续空间来存储元素，插入元素时如果空间不足会扩容，扩容后需要挪动数据

尾部插入是均摊 $O(1)$ 的，因为可能会扩容。尾部删除是 $O(1)$
...
CPU、缓存和编译器优化都可能导致指令乱序，指令重排只保证单线程下的正确性

多线程下，可以使用内存序保证操作之间的顺序

std::atomic 默认使用最强的 std::memory_order_seq_cst，如果没有性能优化的需求，直接使用默认的内存序即可
